...
def dialog(header='', info='', labels=[], types=[], 
           values=[], comments=[]):
    """
    ...
    for l in [types, values, comments]:
        if len(l) != len(labels):
            ...
            does not match the number of desired labeled widgets ({1})""".format(l, len(labels)))
    ...
    root = Tk()
    root.title(header)
    Label(root, text=info, font=infofont, wraplength=300, justify='left', 
          fg='blue').grid(sticky='w', row=0, column=0, columnspan=9, 
                          pady=(5, 20), padx=(5, 5))
    ...
    for i, label in enumerate(labels):
        ...
        Label(root, text=label, font=font).grid(sticky='w', row=i+1, 
                                                column=0, padx=(5, 5))
        # Comment for the input box
        Label(root, text=comments[i], font=font, wraplength=100,
              justify='left').grid(sticky='w', row=i+1, column=8, columnspan=1, 
                                   padx=(5, 5), pady=(5, 5))

        # TEXT ENTRY
        if types[i] == 'e':
            entr[i].grid(sticky='w', row=i+1, column=1, columnspan=7, 
                         pady=(5, 5), padx=(5, 5))
            entr[i].insert(0, values[i])
        # CHECKBOX
        if types[i] == 'c':
            entr[i].grid(sticky='w', row=i+1, column=1, columnspan=1, 
                         pady=(5, 5), padx=(5, 5))
            entr[i].state(['!alternate'])
        # Dropdown Menu
        if types[i] == 'd':
            entr[i].grid(sticky='w', row=i+1, column=1, columnspan=3, 
                         pady=(5, 5), padx=(5, 5))
